...
# TODO Refactor coordinates to nodes, mb use map or zip
# TODO test coordinates to nodes
# TODO test get node to id
...
def create_node_dict(map_dict: dict) -> dict:
    ...
    for item in map_dict["osm"]["node"]:
        new_node = Node(osm_id=int(item['@id']), 
                        lat=float(item['@lat']), 
                        lon=float(item['@lon']))
...
def get_id_from_nodes(node_dict: dict) -> tuple:
    ...
    try:
        ...
        if beg_id not in node_dict or end_id not in node_dict:
            raise Exception("Node Ids are not in the given node_dict")
...
def get_node_from_id(map_dict: dict, node_dict: dict, node_id) -> tuple:
    ...
    if node_id not in node_dict:
        raise Exception("Error: Node id not found")
...
def coordinates_to_nodes(point: Point, node_dict: dict, way_dict: dict, bbox: BoundingBox) -> [Node]:
    ...
    for node in node_dict.values():
        ...
        # If the distance is greater or equal than the max, skip the node
        if distance >= max_pos[1]:
            continue
        ...
        if not abnormal_case:
            closest_nodes[max_pos[0]] = (node, distance)
        
        # Find the new max_pos
        tmp_max_pos = (None, -math.inf)
        ...
        for i in range(AMOUNT_OF_CLOSEST_NODES):
            ...
            if closest_nodes[i][1] > tmp_max_pos[1]:
                tmp_max_pos = (i, closest_nodes[i][1])
        
        if tmp_max_pos != (None, -math.inf):
            max_pos = tmp_max_pos
        
    # Sort the closest node list by increasing distance
    closest_nodes.sort(key=lambda pair: pair[1])
    closest_nodes = [n[0] for n in closest_nodes]
...
def main():
    ...
    # A* search
        ...
        # Get node of beg and end
        
        # Start from beg, using a heuristic based on calculating distance of latlon
        
        # Get the ways that the node is a part of
